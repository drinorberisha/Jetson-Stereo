...
def main():
    ...
    for idx, stereo_img_path in enumerate(stereo_image_files):
        ...
        left_img, right_img = split_stereo_image(stereo_img)
        if left_img is None or right_img is None:
            print(f"Error: Failed to split image {stereo_img_path}")
            continue
    ...
    data = np.load('images/output/stereo_calibration_data.npz')
    ...
    for idx, stereo_img_path in enumerate(stereo_image_files):
        ...
        cv2.imwrite(f'../images/output/rectified_{idx}.jpg', combined_rectified)
